 ...
 */
    public function loadToolConsumer($consumer)
    {
        ...
        $rsConsumer = $this->db->query($sql);
        if ($rsConsumer) {
            while ($row = $rsConsumer->fetch_object()) {
 ...
 */
    public function saveToolConsumer($consumer)
    {
        ...
        if ($ok) {
            if (empty($id)) {
                $consumer->setRecordId($this->db->insert_id);
 ...
 */
    public function loadContext($context)
    {
        ...
        $rs_context = $this->db->query($sql);
        if ($rs_context) {
            $row = $rs_context->fetch_object();
            if ($row) {
...
// Load the nonce
        ...
        $rs_nonce = $this->db->query($sql);
        if ($rs_nonce && $rs_nonce->num_rows === 0) {
          $ok = false;
        }
...
// Load share key
        $id = $this->db->real_escape_string($shareKey->getId());
 ...
 */
    public function saveUser($user)
    {
        ...
        if ($ok) {
            if (is_null($user->created)) {
                $user->setRecordId($this->db->insert_id);
                $user->created = $time;
            }
            $user->updated = $time;
        }
